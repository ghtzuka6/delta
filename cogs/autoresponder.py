...
class ar(commands.Cog):
    ...
    async def autoresponder(self, ctx: commands.Context, subcommand: str=None, trigger: str=None, *, response: str=None) -> None:
     ...
     if subcommand is None:
       e = discord.Embed(color=0x2f3136, title="autoresponder", description="sets an autoresponder the server")
     ...
     if subcommand == "set":
      ...
      check = collection.find_one({"guildid": ctx.guild.id, "trigger": trigger})
      if check is None:
         embed = discord.Embed(color=discord.Color.yellow(), description=f"there isn't an autoresponder with this trigger")
         await ctx.reply(embed=embed, mention_author=False)
         return
      elif check is not None:

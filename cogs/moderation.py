...
class moderation(commands.Cog):
   ...
   async def ban(self, ctx: commands.Context, member: discord.Member=None, *, reason=None):
    ...
    if member == None or member == ctx.message.author:
        e = discord.Embed(color=0xFFEA00, description=f"<:check_warning:956780930066964500> {ctx.author.mention}: you cannot ban yourself")
        await ctx.reply(embed=e, mention_author=False)
        return
    
    if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=0xFFEE00, description=f"<:check_warning:956780930066964500> {ctx.author.mention}: you can't ban {member.mention}")
    ...
    if ctx.guild.premium_subscriber_role in member.roles:
     ...
     async def button1_callback(interaction: discord.Interaction):
        ...
        try: 
          ...
          embe = discord.Embed(color=0x2f3136, description=f"<:icons_hammer:976525276186038312> {member.mention} got banned | {reason}")
         ...
         no = discord.Embed(color=0xFFEE00, description=f"<:check_warning:956780930066964500> {ctx.author.mention}: i don't have enough permissions to do this")
     ...
     try: 
      ...
      embed = discord.Embed(color=0x2f3136, description=f"<:icons_hammer:976525276186038312> {member.mention} got banned | {reason}")
   ...
   async def kick(self, ctx: commands.Context, member: discord.Member=None, *, reason=None):
    ...
    if member.top_role >= ctx.author.top_role and ctx.author.id != ctx.guild.owner.id:
       nope = discord.Embed(color=0xFFEE00, description=f"<:check_warning:956780930066964500> {ctx.author.mention}: you can't kick {member.mention}")
       await ctx.reply(embed=nope, mention_author=False)
       return   
    ...
    if ctx.guild.premium_subscriber_role in member.roles:
     ...
     async def button1_callback(interaction: discord.Interaction):
        ...
        try: 
          ...
          embe = discord.Embed(color=0x2f3136, description=f"<:icons_hammer:976525276186038312> {member.mention} got kicked | {reason}")
          await interaction.response.edit_message(embed=embe, view=None)
          try:
     ...
     try: 
      await member.kick(reason=f"kicked by {ctx.author} - {reason}")
      embed = discord.Embed(color=0x2f3136, description=f"<:icons_hammer:976525276186038312> {member.mention} got kicked | {reason}")
      await ctx.reply(embed=embed, mention_author=False)
      try:
           banned = discord.Embed(color=0xFF0000, title="kick case", description=f"you have been kicked from {ctx.guild.name}\nIf you want to dispute this situation, contact an admin")
           banned.set_thumbnail(url=ctx.guild.icon.url)
